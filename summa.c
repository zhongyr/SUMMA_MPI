...
#define SZ 100		//Each matrix of entire A, B, and C is SZ by SZ. Set a small value for testing, and set a large value for collecting experimental data.
...
void printDiff(double **data1, double **data2, int n, int iListLength, float fListTol)
{
    ...
    for (j = 0; j < n; j++)
    {
        ...
        for (i = 0; i < n; i++)
        {
            double fDiff = abs(data1[j][i] - data2[j][i]);
